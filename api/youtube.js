    ...
    try{
        ...
        try{
            ...
            return Array.isArray(payload?.[1])
                ? payload[1].map(v => String(v || "").trim()).filter(Boolean).slice(0, 20)
                : [];
        } finally {
            clearTimeout(timer);
        }
...
async function fetchGoogleTrendsRelatedQueries(seedKeyword){
    ...
    try{
        ...
        try{
            ...
            if(!response.ok) return [];
            const top = Array.isArray(payload?.related_queries?.top) ? payload.related_queries.top : [];
            const rising = Array.isArray(payload?.related_queries?.rising) ? payload.related_queries.rising : [];
            return [...top, ...rising]
                .map(item => ({
                    keyword: String(item?.query || "").trim(),
                    trendValue: Number.isFinite(Number(item?.extracted_value)) ? Number(item.extracted_value) : null,
                    trendLabel: String(item?.value || "").trim()
                }))
        ...
        console.warn("[TubeX] Google Trends related queries indisponível:", e?.message || e);
        return [];
    }
}
...
const googleSearchPromise = fetchGoogleVerification(keyword);

// Related-search enrichment is deliberately best-effort. The core YouTube
// analysis must never remain blocked by Google/Autocomplete/Trends outages.
// Each auxiliary request already has its own AbortController; this outer
// deadline guarantees the /youtube response can still complete.
const relatedEnrichment = Promise.all([
    Promise.resolve(googleSearchPromise),
    fetchYouTubeAutocompleteSuggestions(keyword),
    fetchGoogleTrendsRelatedQueries(keyword)
]).then(([googleSearch, autocompleteSuggestions, trendQueries]) => ({
    googleSearch,
...
const enrichmentTimeout = new Promise(resolve => {
    setTimeout(() => resolve({
        googleSearch: null,
        autocompleteSuggestions: [],
        trendQueries: []
...
const {
    ...
    autocompleteSuggestions,
    trendQueries
} = await Promise.race([relatedEnrichment, enrichmentTimeout]);
...
const relatedKeywords = buildRelatedKeywords(
    items,
    keyword,
    requestedRelatedLimit,
    autocompleteSuggestions,
    trendQueries,
    finalVolume
);
